...
def send_simple_message(text):
    return requests.post(
        ...
        data={"from": "Excited User <mailgun@clcrawler.com>",
              "to": ["Bzeaiter@gmail.com", "YOU@clcrawler.com"],
              "subject": "New car",
              "text": text})
...
def carscrap(link):
            ...
            while(1):
                proxies=getproxy()
                try:
                    source = requests.get(the_link,timeout=4.5,proxies=proxies,headers = headers)
                    ...
                    if(dicti[link] != cities[link]):
                        for ded in dicti[link]:
                            if (cities[link]==[] and dicti[link]!=[]):
                                ...
                                carurl = 'https://' + link + '.craigslist.org' + href
                                print("URL: ", carurl)
                               # carurl = 'https://' + link + '.craigslist.org/' + 'cto/d' + '/randomname/' + ded + '.html'
                                ...
                                #slen = urlic.find('cto')
                                # carurl = 'https://' +  link + '.craigslist.org' + urlic[0:slen] + 'cto/' +  'd/' + 'randomname/' + ded + '.html'
                                #print("URL: ", carurl)
                                bab = soup.find(attrs={"data-pid" : int(ded)})
                                price = bab.find('span', class_='result-price').text
                                babic = soup.find('a',attrs={"data-id" : int(ded)} )
...
while(1):
    ...
    for car in Cars:
        ...
        #print("Cities:", cities)
        if(cities=='{}'):
            print("Car model doesn't have cities.")
            continue
